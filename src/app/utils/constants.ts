...
import { IProviderSetting, ProviderInfo } from "../types/model";
...
const staticModels: ModelInfo[] = PROVIDER_LIST.map(
  (p) => p.staticModels
).flat();
  ...
  try {
    ...
    return data.map((m: any) => ({
      ...
      label: `${m.display_name} - in:$${m.pricing.input.toFixed(
        2
      )} out:$${m.pricing.output.toFixed(2)} - context ${Math.floor(
...
const getOllamaBaseUrl = (settings?: IProviderSetting) => {
  const defaultBaseUrl = settings?.baseUrl || "http://localhost:11434";
  ...
  return data.data
    ...
    .map((m) => ({
      ...
      label: `${m.name} - in:$${(m.pricing.prompt * 1_000_000).toFixed(
        2
      )} out:$${(m.pricing.completion * 1_000_000).toFixed(
        2
      )} - context ${Math.floor(m.context_length / 1000)}k`,
      provider: "OpenRouter",
      maxTokenAllowed: 8000,
    }));
}
